...
# 各々の回数をカウント
tmp = ['ratio_W', 'ratio_E', 'ratio_WE', 'ratio_WWE', 'ratio_Exit', 'ratio_W_any']
colnames = tmp[:]
...
characters = ['W', 'E', 'WE', 'WWE', 'E|WE|WWE', 'W|E|WE|WWE'] * 5  # first, second, last8, last3, full
...
for year in range(2015, 2019):
    ...
    for div, team_id in zip(team_df_year["div"], team_df_year["team_id"]):
        ...
        if is_2stage:
            with open(f"../input/event/event_we_{year}_{div}_{team_id}_s1.json") as f:
                first_dic = json.load(f)
            with open(f"../input/event/event_we_{year}_{div}_{team_id}_s2.json") as f:
            ...
            with open(f"../input/event/event_we_{year}_{div}_{team_id}.json") as f:
                dic = json.load(f)
    ...
    df = pd.concat(dfs)
    df.to_csv(f'../input/orgn/event_we_{year}.csv', index=False)