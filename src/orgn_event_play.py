...
# 各々の回数をカウント
tmp = ['ratio_full_play', 'ratio_out_play', 'ratio_in_play', 'ratio_inout_play', 'ratio_bench_play', 'ratio_susp_play', "ratio_part_play"]
colnames = tmp[:]
...
characters = ['○', '▲', '▽', '◆', 'B', '×', '▲|▽|◆'] * 5  # first, second, last8, last3, full, part
...
for year in range(2015, 2019):
    ...
    for div, team_id in zip(team_df_year["div"], team_df_year["team_id"]):
        ...
        if is_2stage:
            with open(f"../input/event/event_play_{year}_{div}_{team_id}_s1.json") as f:
                first_dic = json.load(f)
            with open(f"../input/event/event_play_{year}_{div}_{team_id}_s2.json") as f:
            ...
            with open(f"../input/event/event_play_{year}_{div}_{team_id}.json") as f:
                dic = json.load(f)
    ...
    df = pd.concat(dfs)
    df.to_csv(f'../input/orgn/event_play_{year}.csv', index=False)