...
def run(fold):
    ...
    # log
    mlflow.log_param(f'Fold{fold}_params', model.params)
    mlflow.log_param(f'Fold{fold}_iteration', model.best_iteration)
    mlflow.log_param(f'Fold{fold}_best_score', model.best_score) 
...
if __name__ == "__main__":
    ...
    df = df[features+["label", "kfold"]]

    # target_encoding
    #df = utils.mean_target_encoding(df)
